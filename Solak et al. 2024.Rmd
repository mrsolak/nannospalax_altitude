...
*9D - Venn Diagrams*
```{r warning=F, message=F}
#Tutorial is here : https://microbiome.github.io/tutorials/core_venn.html
...
table(meta(PS2_cor)$pop, useNA = "always")

pseq.rel <- microbiome::transform(PS2_cor, "compositional")

#Make a list of DiseaseStates.

disease_states  <- unique(as.character(meta(pseq.rel)$altitude))
...
for (n in disease_states){ # for each variable n in DiseaseState
    ...
    ps.sub <- subset_samples(pseq.rel, altitude == n) # Choose sample from DiseaseState by n
...
#do it for sampling localities
library(ggVennDiagram)
ggvenn_pq(physeq = PS2_cor, fact = "pop")
...
p=ggplot(phenotypes_melted, aes(x = variable, y = value, fill = Alt)) +
  geom_boxplot() +
  labs(title = "Bacterial Phenotypes Boxplot by Altitude",
       x = "Phenotypes",
       y = "Values",
       fill = "Altitude") +
...
###############################
exclusive_ASVs <- NULL
for (i in levels(sample_data(PS2_cor)$altitude)) {
    ds_temp <- DESeq(ds, test = "LRT", reduced = ~1, fitType = "parametric")
    sig_ASVs <- rownames(results(ds_temp, tidy = TRUE)[results(ds_temp, tidy = TRUE)$padj < 0.05, ])
    if (i == levels(sample_data(PS2_cor)$altitude)[1]) {
        exclusive_ASVs <- sig_ASVs
    } else {
        exclusive_ASVs <- intersect(exclusive_ASVs, sig_ASVs)
    }
}
