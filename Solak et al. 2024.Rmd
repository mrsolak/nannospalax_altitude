...
SSUM<-sample_sums(PHYLOSEQ2) #Returns the total number of individuals observed from each sample
...
#Compare duplicates
dupl.1<-prune_samples(duplicated(sample_data(PHYLOSEQ2)$sample_ID)==F,PHYLOSEQ2) #47 samples
dupl.2<-prune_samples(duplicated(sample_data(PHYLOSEQ2)$sample_ID)==T,PHYLOSEQ2) #47 samples
...
p=plot_bar(PS_alt, fill = "Phylum") + geom_bar(stat="identity", position="stack") +scale_fill_manual(values=c25)
...
##Beta Diversity: Plot the PCoA using the weighted UniFrac as distance (takes account phylogeny):
...
p=plot_ordination(PS2_cor, ordination, color="altitude") + theme(aspect.ratio=1)+ ggtitle("Beta Diversity. PCoA using the Weighted Unifrac distance")+ scale_y_reverse() + geom_point(size=2)
...
p=plot_ordination(PS2_cor, ordination, color="altitude", shape = "pop") + theme(aspect.ratio=1)+ ggtitle("Beta Diversity. PCoA using the Bray-Curtis distance") + geom_point(size=2)

plot_ordination(PS2_cor, ordination, color="altitude", shape = "pop") + theme(aspect.ratio=1, legend.text = element_text(size = 12)) +geom_point(size=3) + scale_x_reverse() + scale_y_reverse() +  scale_shape_manual(values=c(0,4,21,3,15,17))


## PERMANOVA##
adonis2(wunifrac_dist ~ sample_data(PS2_cor)$altitude) #significant
adonis2(bray_dist ~ sample_data(PS2_cor)$altitude)     ##significant
...
model_cat<-glmmTMB(Axis.2~altitude+(1|pop),data=ORD_df)
...
#should be definetly used - models with PCoA capture only relatively low fraction of variation in the microbiota (11 and 10% for the first and the second axes, respectively)
#but information you can see in the MDMR output are limited
#you probably conduct separate analyses to compare high ~ middle, low ~ middle etc samples. 

#altitude mdmr
mdmr.res <- MDMR::mixed.mdmr(~altitude+
                               (1|pop),
                             data =ORD_df,D = DIST)
summary(mdmr.res)
...
GG<-ggplot(TT,aes(y=Abundance/Seq.tot,x=pop))+geom_boxplot(outlier.shape = NA)+geom_jitter()+facet_wrap(~new,scales = "free")

GG$data$pop<- as.character(GG$data$pop)
GG$data$pop<-factor(x = GG$data$pop, levels = unique(GG$data$pop[order(as.character(GG$data$alt2))]))

GG+ ggtitle("OTU differential abundance testing with DESeq2")
...
random_tree = rtree(ntaxa(PS2_cor), rooted=TRUE, tip.label=taxa_names(PS2_cor))
PS2_cor = merge_phyloseq(PS2_cor, random_tree)
...
model_mass<-glmmTMB(Shannon~weight+(1|pop),data=RICH)
...
ALL=readgenotypedata("D:/MolSys/2021 Microsats/2ndrun (1)/related/ALL_related.txt")
ERG=readgenotypedata("D:/MolSys/2021 Microsats/2ndrun (1)/related/eregli2.txt")
DAR=readgenotypedata("D:/MolSys/2021 Microsats/2ndrun (1)/related/darbogaz2.txt")
KAR=readgenotypedata("D:/MolSys/2021 Microsats/2ndrun (1)/related/karagol2.txt")
KIZ=readgenotypedata("D:/MolSys/2021 Microsats/2ndrun (1)/related/kiziltepe2.txt")
MAD=readgenotypedata("D:/MolSys/2021 Microsats/2ndrun (1)/related/madenkoy2.txt")
ULU=readgenotypedata("D:/MolSys/2021 Microsats/2ndrun (1)/related/ulukisla2.txt")
...
#XX_sim=familysim(data$freqs,100)
#XX_sim_co=coancestry(data, lynchrd=2, wang=2)

load("D:/MolSys/2021 Microsats/2ndrun (1)/oleksiy related/Dr_co.gz")
load("D:/MolSys/2021 Microsats/2ndrun (1)/oleksiy related/Dr_sim_co.gz")

load("D:/MolSys/2021 Microsats/2ndrun (1)/oleksiy related/Kr_co.gz")
load("D:/MolSys/2021 Microsats/2ndrun (1)/oleksiy related/Kr_sim_co.gz")

load("D:/MolSys/2021 Microsats/2ndrun (1)/oleksiy related/Er_co.gz")
load("D:/MolSys/2021 Microsats/2ndrun (1)/oleksiy related/Er_sim_co.gz")

load("D:/MolSys/2021 Microsats/2ndrun (1)/oleksiy related/Kz_co.gz")
load("D:/MolSys/2021 Microsats/2ndrun (1)/oleksiy related/Kz_sim_co.gz")

load("D:/MolSys/2021 Microsats/2ndrun (1)/oleksiy related/Md_co.gz")
load("D:/MolSys/2021 Microsats/2ndrun (1)/oleksiy related/Md_sim_co.gz")


load("D:/MolSys/2021 Microsats/2ndrun (1)/oleksiy related/Ul_co.gz")
load("D:/MolSys/2021 Microsats/2ndrun (1)/oleksiy related/Ul_sim_co.gz")


write.table(Er_co$relatedness, file = "D:/MolSys/2021 Microsats/2ndrun (1)/related/relatedness tables/ERG.csv")
write.table(Ul_co$relatedness, file = "D:/MolSys/2021 Microsats/2ndrun (1)/related/relatedness tables/ULU.csv")
write.table(Dr_co$relatedness, file = "D:/MolSys/2021 Microsats/2ndrun (1)/related/relatedness tables/DAR.csv")
write.table(Md_co$relatedness, file = "D:/MolSys/2021 Microsats/2ndrun (1)/related/relatedness tables/MAD.csv")
write.table(Kz_co$relatedness, file = "D:/MolSys/2021 Microsats/2ndrun (1)/related/relatedness tables/KIZ.csv")
write.table(Kr_co$relatedness, file = "D:/MolSys/2021 Microsats/2ndrun (1)/related/relatedness tables/KAR.csv")
