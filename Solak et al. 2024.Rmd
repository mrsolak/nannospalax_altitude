...
'%!in%' <- function(x,y)!('%in%'(x,y))
PS2_cor= subset_samples(PS2_cor, altitude %!in% "out"==T)

PHYLOSEQ2.rare=phyloseq_mult_raref_avg(PS2_cor, iter = 100, parallel=T)

save(PHYLOSEQ2.rare, file="D:/Rwd/phyloseq_fin/PHYLOSEQ2.rare.R")
...
# 4- mixed models
rm(list = ls())
load("D:/Rwd/phyloseq_fin/PHYLOSEQ2.rare.R")
...
PS_hor<-phyloseq_mult_raref_avg(PS_hor, iterations=100) 
...
PHYLOSEQ2.rare<-phyloseq_mult_raref_avg(PS_hor, iterations=100) 
PHYLOSEQ2.trans<-transform_sample_counts(PHYLOSEQ2.rare,function(x) x/sum(x))
DIST<-vegdist(otu_table(PHYLOSEQ2.trans))
ORD<-ordinate(PHYLOSEQ2.trans,method="PCoA",distance = DIST)
ORD_df<-plot_ordination(PHYLOSEQ2.trans,ORD,justDF = T)
