...
p=plot_ordination(PS2_cor, ordination, color="altitude", shape = "pop") + theme(aspect.ratio=1)+ ggtitle("Beta Diversity. PCoA using the Bray-Curtis distance") + geom_point(size=5)+ theme(text = element_text(size = 20),legend.text=element_text(size=20))+theme_classic()
...
PS_hor<-rarefy_even_depth(PS_hor)
...
PHYLOSEQ2.rare<-rarefy_even_depth(PS_hor)
PHYLOSEQ2.trans<-transform_sample_counts(PHYLOSEQ2.rare,function(x) x/sum(x))
DIST<-vegdist(otu_table(PHYLOSEQ2.trans))
ORD<-ordinate(PHYLOSEQ2.trans,method="PCoA",distance = DIST)
ORD_df<-plot_ordination(PHYLOSEQ2.trans,ORD,justDF = T)
